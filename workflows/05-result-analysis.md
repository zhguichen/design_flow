...
对合成数据集做描述统计 + 开放题主题编码 + 分群交叉分析，产出洞察与设计启示。报告显式区分"模拟数据显示的模式" vs "需真实用户验证的假设"。
...
- `runs/<时间戳>/survey.json` + `archetypes.json` + `respondents.jsonl`
...
4. **设计启示**：从模式推出对设计决策的启示。
5. **置信度与局限**：声明 合成样本 / 样本量 / 置信度 / 仅供预调研；**显式区分**：
   - "模拟数据显示的模式"（数据里看到的）
