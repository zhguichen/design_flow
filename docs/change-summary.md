...
> 当前合同：不再生成独立预测文件。archetype、mechanism、task friction 和 persona 都只保存原因与可观察事实，不保存逐题预测方向、答案阈值或排序规则。
...
这些事实由 WF3 写进 persona；具体答案由 WF4 独立生成，上游不生成预测方向。
...
- **需真人确认的问题**：哪些观察不能从合成场景外推，需要真人访谈或真实问卷校准。
...
| 分析结果容易被误当真实证据 | WF5 强制标注合成样本，并区分模拟模式、可能解释、需真人确认的问题 |

## 还没有解决什么

这次修改不会证明合成样本就等于真实用户，也不会证明某类人真实存在。它只能让场景依据更明确、更可追溯，并指出哪些问题需要真人研究。
...
- `report.md` 是否区分模拟数据、可能解释、需真人确认的问题。
