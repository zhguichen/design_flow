...
- 用户已经有问卷文本、截图、表格或 `survey.json`，需要转成下游可用的标准结构。

不适用：用户想"发现未知问题"——那是访谈的活，问卷只确认和量化已知构念，不负责揭示未知。遇到这种请求，说明原因并建议先做定性访谈。

## 开头先分流

WF1 不默认替用户写问卷。先识别输入模式：

| 模式 | 用户输入 | 目标 | 原则 |
|---|---|---|---|
| A. 已有问卷 | 粘贴题目、上传问卷、给表格/截图/文档，或说“我已有问卷” | 把现有问卷规范化为 `survey.json` | 尽量保留用户原题意；只修明显偏差、题型错误、选项缺口和题序问题 |
| B. Agent 生成问卷 | 用户只有设计题目、方案、研究问题，或说“帮我生成问卷” | 先追问关键信息，再设计问卷 | 不直接生成一堆“是否愿意”题；先建立行为、情境、痛点和边界构念 |
| C. 已有 `survey.json` | 用户提供标准文件或 run 目录 | 校验后进入 WF2 | 只补缺失字段或结构错误，不重写问卷 |

### A. 已有问卷：规范化，不重写

用户已有问卷时，先把题目解析成结构化草稿，再做方法修订：

1. 保留原始题意和用户措辞，除非存在明显引导、双重问题、术语不清、选项不互斥或量表方向混乱。
2. 为每题判断 `type`、`options`、`scale`、`required` 和 `construct_measured`。
3. 把结果变量（满意度 / 使用意愿 / 推荐意愿 / 付费意愿）标出来，但不得把它们变成 persona 背景。
4. 若缺少研究问题或目标人群，先追问；若只是缺少少量题型/选项信息，可基于题面合理补齐并在门 1 说明。
5. 输出 `survey.json` 后展示“保留了什么、修改了什么、为什么修改”，再让用户确认。

### B. Agent 生成问卷：先问设计意图

用户没有问卷时，问卷生成前必须先弄清设计目的。若上下文不足，最多集中追问 3-5 个最关键问题，优先问：

- **设计对象是什么**：产品、服务、空间、视觉系统、交互流程、装置、品牌、课程项目等。
- **目标人群是谁**：必须包含行为或经历，不只写年龄/性别/学历。
- **使用场景是什么**：用户在什么时间、地点、任务或压力下遇到问题。
- **你的方案/设想是什么**：已有功能、形态、服务流程、材料、技术或概念方向。
- **最想验证什么**：痛点、功能优先级、接受度、使用边界、价格、信任、安全、审美、空间体验等。

信息足够后再生成问卷。题目要服务于设计决策：先问行为和场景，再问痛点/优先级/边界，最后测满意度、使用意愿或付费意愿。不要让全卷被“是否愿意 xxx”统治。

### C. 已有 `survey.json`：校验后继续

用户提供标准 `survey.json` 或要求从已有 run 继续时，先运行 WF1 校验；通过则不重新生成问卷，直接进入 WF2。缺字段时只补必要字段，并在门 1 说明补了什么。
...
- **用来干什么**（`purpose`）：固定为 `pre-research`，表示方向判断、问卷预检和合成场景比较。如果用户想做真实假设验证或发表级研究，说明本工具与合成样本达不到该标准，建议改用真实样本研究；不要为迎合用户改写 `purpose`。
...
`simulation_n` 是生成多少条合成记录的计算预算，不是真实样本量，不对应置信区间或总体代表性。WF1 尚不知道最终有多少 archetype，因此不在这里单独追问或确认规模。

用户已经主动给出数字或预算上限时，记录下来交给 WF2 核对；否则填 `null`，并在 `simulation_n_rationale` 写明“待 WF2 根据最终 archetype 数统一建议并确认”。最终只在 WF2 Phase C 完成后的 audience package 门禁确认一次。
...
先运行 `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/validate_run.py" runs/<时间戳>/ --stage wf1` 检查文件结构、id、题型和量表端点。以下只保留需要模型判断的方法质量：
...
继承 `SKILL.md` 的跨 workflow 红线（合成样本标注、结果变量不作背景）。本步独有：`purpose` 只能写 `pre-research`。用户要求发表级或现实验证结论时应明确拒绝外推。
...
需要具体产出示例时按需读取 `references/output-examples.md`，不要为正常执行预加载示例。
