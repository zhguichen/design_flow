...
执行 WF4 前先向用户显示：“WF4 需要开启 subagent 并行进行，让每个作答模型只看到单个 persona、问卷与允许的机制信息，避免跨人物答案互相影响。”这是一条执行说明，不新增确认门。
...
WF4 只能读取 persona 的**可观察背景、经历、资源、情境、能力、心理倾向和 mechanism_trace**，不读取 `task_frictions.json`。
...
根据 persona 的实际处境逐题判断；同 archetype 个体只在经历和约束确实相同的地方才该一致，不同属性应产生有依据的差异。不得为了制造更明显的分群差异而重生成或纠偏。
...
- **persona_answer_match**：答案能追溯到该 persona 的处境、经历、资源、能力、性格倾向。不能以分群差异是否明显来评分——标准是“像不像这个人”，不是“结果够不够好看”。
...
继承 `SKILL.md` 的跨 workflow 红线（结果变量与答案方向红线）。本步独有：**不模拟无效作答/随机/恶意反填**；**不传入摩擦分数或作答规则**——subagent 只凭 persona 背景故事答题，不执行查找表。
