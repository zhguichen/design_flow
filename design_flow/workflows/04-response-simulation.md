...
- `runs/<时间戳>/respondents.jsonl`：受访者五层信息 + mechanism_trace。
...
- `task_frictions.json`：摩擦分数和作答规则不传入 subagent。这是方案 A 的核心——subagent 不知道"F2=2 应该打 1-2 分"，只知道这个人的性格、处境、经历，让它像真人一样凭感觉答题。

## 方法

### 作答模式：凭 persona 背景故事答题，不执行规则表

每个受访者独立、逐题作答。**subagent 只拿到该 persona 的五层背景信息（基础身份、社会处境、心理倾向、行为习惯、作答风格）和 mechanism_trace（心理机制描述），不拿到任何摩擦分数或作答规则。** 
...
优先在新的隔离 context / subagent 中运行本步，每个 subagent 只传入：该 persona 的五层信息 + mechanism_trace + 所属 archetype 的 behavior_mechanisms + survey.json。不传入跨 persona 的比较信息、不传入摩擦分数、不传入任何预测方向。
...
```json
{
  ...
  "note": "合成样本 / 仅供预调研 / 反映模拟质量不等于研究效度 / 方案A：不传摩擦分数和作答规则，subagent 凭 persona 背景故事独立作答"
}
```
...
`reasoning` 关键题必给，从 persona 的处境/经历/性格推导，不引用分数；`mechanism_id` 关键题建议填，来自 respondent 的 `mechanism_trace.mechanism_ids`；`hypothesized_latent_motive` 由机制链推导，只在关键题必填；`uncertain` 为 true 时 answer 填"不确定/不了解/不适用"或量表中点。
...
- **覆盖度与强类型**：`responses.jsonl` 每行含 `respondent_id`+`answers`，与 `respondents.jsonl` 一一对应；`answers` 覆盖 `survey.json` 所有题；每个 answer 值符合强类型表。
- **关键题可追溯**：结果变量类的题有非空 `reasoning`，能追溯到 persona 的具体处境、经历或性格倾向——不是引用分数。
- **无无效作答模式**：没有直线作答，没有除非 persona 真的啥都没经历、否则全部 uncertain 的情况。
- **假设隔离正确**：responses 不含 hypothesis id、`predicted_pattern` 或任何来自 `hypotheses.json` 的内容；`responses_meta.blinding` 含 `level`/`hypotheses_loaded=false`/`task_frictions_loaded=false`/`allowed_inputs`。
- **meta 完整**：`responses_meta.json` 含 `count`/`quality`/`invalid_patterns_detected`/`note`，`note` 含"合成样本"、"预调研"和"方案A"。

## 该停下来问的情况

- 缺 `respondents.jsonl`/`behavior_mechanisms.json`/`survey.json` 任一个 → 回对应上游步骤。
