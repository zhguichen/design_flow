---
name: design-flow
description: 面向设计类学生/从业者的问卷预调研 Skill——设计问卷、反推人群、用人文社科/心理学行为机制和任务摩擦模型推导真实合理存在的人群类型、生成 persona、LLM 模拟填写、分析结果，产出合成样本替代低质量互填。
---
...
触发。目标：产出可信、足量、结构合理、带机制依据和任务摩擦依据的合成问卷样本（人群画像 + 行为机制推导 + 任务摩擦推导 + 模拟数据集 + 分析报告），替代低质量互填。
...
2.5 behavior-mechanism-mapping 行为机制映射   → runs/<ts>/behavior_mechanisms.json
2.6 task-friction-mapping      任务摩擦映射   → runs/<ts>/task_frictions.json
...
| 触发 | 加载 | 产出 |
|---|---|---|
| 用户给研究问题 / 目标人群，需设计问卷；或 `survey.json` 不存在 | `workflows/01-survey-design.md`（可参考已安装的 Anthropic `user-research` 做方法适配） | `survey.json` |
| `survey.json` 就绪，需反推人群原型 | `workflows/02-audience-inference.md` | `archetypes.json` |
| `archetypes.json` 就绪，需说明这些类型为什么真实合理存在 | `workflows/025-behavior-mechanism-mapping.md` + `references/behavior-mechanism-library.md` | `behavior_mechanisms.json` |
| `behavior_mechanisms.json` 就绪，需说明任务/痛点/麻烦环节如何推导 | `workflows/026-task-friction-mapping.md` + `references/task-friction-framework.md` | `task_frictions.json` |
| `archetypes.json` + `behavior_mechanisms.json` + `task_frictions.json` 就绪，需展开成具体受访者 | `workflows/03-persona-generation.md` | `respondents.jsonl` + meta |
...
**串联用法**：用 `/design-flow:run-pipeline` 串 1→2→2.5→2.6→3→4→5（步间暂停）。

每步须满足该 workflow 的验收标准（可机械检查）再进下一步；触发 Stop 条件则停下追问，不臆造。

## 跨 workflow 红线

任何输出（人群画像 / 行为机制 / 任务摩擦 / 模拟数据集 / 分析报告）都必须标注：**合成样本 / 样本量 / 置信度 / 仅供预调研**。结果变量（满意度 / 使用意愿 / 推荐意愿 / 付费意愿）不能作为 persona 背景设定，只设定影响它们的原因。人群类型必须能追溯到“环境/事件 → 处境/限制 → 行为机制 → 任务摩擦 → 表层需求/潜在动机 → 作答影响”的机制链；没有机制支撑或任务摩擦支撑的可能性不进入样本。

## Prerequisites

- 无外部依赖（skill 主体纯 Markdown，LLM 驱动）。
- 如果 Anthropic `user-research` skill 已安装，可在 WF1 作为研究方法/问卷设计顾问使用；最终仍必须输出本 skill 的 `survey.json` schema。
...
- `behavior_mechanisms.json` 覆盖每个 archetype，并标注理论依据、表层需求、潜在动机、纳入/排除边界和验证追问。
...
- **只生成“看起来像人”的 persona**：没有机制链会让样本像 AI 编故事。必须先做 WF2.5，用研究逻辑说明为什么这类人合理存在。
- **痛点题凭性格乱猜**：问“哪个环节更麻烦”时，不能由性格直接作答。必须先做 WF2.6，用任务摩擦模型把生活情境、行为经验、环境限制、资源能力映射到具体痛点。
- **把所有可能性都塞进样本**：会冲淡结果。只纳入有机制支撑、影响回答、现实自洽的类型；弱机制类型要合并、标低置信或排除。
